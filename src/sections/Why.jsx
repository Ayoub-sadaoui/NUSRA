  ...
  return (
    <section className="flex max-md:flex-col max-lg:hidden relative max-lg:items-center lg:items-center  h-screen md:flex-row  px-20 sm:pt-20 ">
      <div className="   sm:relative z-10 flex items-center md:items-end flex-col   max-sm:mt-[120px]  md:w-[50%] max-md:px-10 ">
        ...
        <p className="max-sm:hidden text-neutral sm:text-secondary text-center md:text-right font-Aljazeera text-xl lg:text-3xl mt-4 ">
          في الوقت الحالي الذي نعيشه و الحرب المستمرة, يزيد الواقع سوءًا يومًا
          بعد يوم، حيث تتسارع أعداد القتلى وتضعف صفوف المدافعين, إضافة إلى قصف
          الطائرات من فوق, فإن الجوع و البرد أصبح يخطف أرواح عشرات الأطفال و
          الأبرياء يوميا, الشيء الذي يوجب على كل شخص المساهمة بأي طريقة يجدها في
          سبيل إحداث تغيير.
        </p>

        <div className="grid justify-center mt-20">
          <button className="lg:px-5 sm:px-3 sm:text-xl  py-2  flex  items-center justify-center  rounded-lg border-[3px]    border-primary  text-primary  font-Aljazeera lg:text-3xl">
            <svg
              className="mr-2"
              width="25"
              height="18"
